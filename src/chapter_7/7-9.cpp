...
bool Table::is_declared(string var)
{
	for (int i = 0; i < var_table.size(); ++i)
...
double Table::declare(string s, double val, bool var = true)
{
...
Token Token_stream::get()
{
	...
	default:
		if (isalpha(ch)) {
			...
			if (s == helpkey) return Token(help_lower);
			return Token(name,s);
		}
		error("Bad token");
	}
	...
	// k will be "let" or "con"(stant)
{
    ...
    double d = expression();
	symbol_table.declare(var_name,d,k.kind==let);
    return d;
}
...
{
	try {
		symbol_table.declare("pi", 3.1415926535, false);
		symbol_table.declare("e", 2.7182818284, false);
		calculate();
		return 0;
	}
	catch(exception& e) {
